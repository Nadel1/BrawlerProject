...
public class RumbleEffect : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //calculation of the rotation rumble using perlin noise
        transform.localRotation = Quaternion.Euler(new Vector3(
            maximumAngularShake.x * Mathf.PerlinNoise(seed, Time.time * frequency) * 2 - 1,
            maximumAngularShake.y * Mathf.PerlinNoise(seed + 1, Time.time * frequency) * 2 - 1 ,
            maximumAngularShake.z * Mathf.PerlinNoise(seed + 2, Time.time * frequency) * 2 - 1 
        )*shake);
